...
1. **Index-Update-and-Expunge**: This model pulls current FHIR resources (could use `_elements` [parameter](https://hl7.org/fhir/search.html#elements) to limit results to just identifiers), and indexes the `.identifier` values found. Then when updating the new VIA feed data, the index marks which `.identifier` were refreshed. At the end, we can know which `.identifier` values were not updated. We can pull them by `.identifier`, change the status to `entered-in-error`, and update. -- Note: Does not require moving to new HAPI Server.
